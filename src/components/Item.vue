<template>
  ...
  <div class="button_card" v-if="role_id==1">
    <button class="redact"><router-link class="router-redact" :to="{ name: 'Redact_item', params: { id: props.item.id, name: props.item.name, type_id: props.item.type_id, cost: props.item.cost, weight: props.item.weight, presence_of_battery_id: props.item.presence_of_battery_id, conducts_electricity_id: props.item.conducts_electricity_id  } }">РЕДАКТИРОВАТЬ</router-link></button>
